    ...
    : (([Plugin, ZLibrary]) => {
      ...
      const LibModules = new class LibraryModules {
        ...
        get UserSettingsProtoStore() {
          return WebpackModules.getByProps("getGuildFolders", "getGuildRecentsDismissedAt");
        }
        ...
        get Timeout() {
          const { V7 } = WebpackModules.getModule(
            (m) =>
        ...
        get DeveloperExperimentStore() {
          return WebpackModules.getModule(m => m?.Z?.hasOwnProperty("isDeveloper"));
        }
        get SettingView() {
        ...
        get AuthenticationStore() {
          return ((allModulesExports) => {
            webpackChunkdiscord_app.push([
              [Math.random()],
              {},
              (r) => (allModulesExports = Object.values(r.c).map(m => m?.exports)),
            ]);
        ...
        get WebRTCUtils() {
          const { prototype } = WebpackModules.getByPrototypes("updateVideoQuality");
          return prototype;
        }
        get TextTags() {
          return WebpackModules.getModule((m) => m?.render?.toString().includes(".titleId"));
        }
        ...
        get Embed() {
          const { prototype } = WebpackModules.getByPrototypes('renderSuppressButton');
          return prototype;
        }
        get SpotifyStore() {
        ...
        get ChannelMemberStore() {
          return WebpackModules.getModule((m) => m?.getProps?.toString().includes("groups"));
        }
        get GuildPrototype() {
        ...
        get BannerClasses() {
          return Object.assign({},
            WebpackModules.getByProps("settingsBanner", "profileBannerPremium"),
      ...
      const libraryUtils = new class LibraryUtils {
        ...
        MakeSubModuleWriteable(mainModule, subModule) {
          let writeableSubModule = Object.assign(Object.create(Object.getPrototypeOf(mainModule?.[subModule])), mainModule?.[subModule]);
      ...
      const spotifyApi = new class SpotifyApi {
        ...
        async addToQueue({ title, uri, showToast }) {
          ...
          const res = await fetch(`${this.URL}/queue?uri=${uri}`, { headers: { authorization: `Bearer ${accessToken}` }, method: 'POST' });
          if (res.ok && showToast) Toasts.show(
            `Queued: ${title}`,
            {
        ...
        async play({ title, uri, addToQueue, showToast }) {
          ...
          if (addToQueue) {
            ...
            const res = await fetch(`${this.URL}/next`, { headers: { authorization: `Bearer ${accessToken}` }, method: 'POST' })
            if (res.ok && showToast) Toasts.show(
              `Now playing: ${title}`,
              {
